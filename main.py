...
def clinterface():
    ...
    parser = argparse.ArgumentParser(   prog='LmBackuper',
    ...
    def run(self):
        for routine in self.args['r']:
            ...
            if self.args['compress']==False:
                copy_mgr.copy_all(verify=self.args['verify'],timestamp=self.args['timestamp'])                
            elif self.args['compress']==True:    
                copy_mgr.zip_all(verify=self.args['verify'], timestamp=self.args['timestamp'])
...
class CopyMgr():
    ...
    def check_paths(self):
        ...
        def check_paths_in_list(paths_list,path_type=""):
            ...
            for path in paths_list:
                ...
                if not exists:
                    ...
                    self.paths_ok=False
                    raise Exception('following {} path doesn\'t exist: {}'.format(path_type,path))
                elif exists:
                    if path_type=='destination' and not os.path.isdir(path):
                        self.paths_ok=False
                        raise Exception('following destination path is supposed to be a directory and it is not: {}'.format(path))
                    else:
        ...
        try:
            ...
            logging.exception('error while checking the paths provided; while this error present files will not be allowed to be copied')
    ...
    def copy_all_verify(self,timestamp=''):
        ...
        for path_s in self.source_paths:
            logging.debug('verifying source path %s: %s', self.source_paths.index(path_s)+1, path_s)
            logging.debug('source path %s will be checked for all destination folders (total amount = %s)', self.source_paths.index(path_s)+1,len(self.destination_paths))
                ...
                for root, dirs, files in os.walk(path_s):
                    ...
                    for path_d in self.destination_paths:
                        ...
                        if root!=path_s:
                            ...
                            root_d=os.path.join(path_d,my_basename,root[-string_length:])                        
                        else:
                            my_basename=timestamp+str(os.path.basename(path_s))
                            root_d=os.path.join(path_d,my_basename)
                        logging.debug('    destination root %s: %s', src_root_no, root_d)
                        if not os.path.exists(root_d):
                            logging.error('    this directory was not created: %s', root_d)
                            err_count+=1
                        else:
                            ...
                            for file in files:
                                logging.debug("    source file %s", os.path.join(root,file))
                                logging.debug("    destination file %s", os.path.join(root_d,file))
                                if not os.path.exists(os.path.join(root_d,file)):
                                    logging.error('    this file was not created: %s', dir)
                                    err_count+=1
                logging.debug('finished verifying source path %s: %s', self.source_paths.index(path_s)+1, path_s)
        ...
        if err_count>0:
            logging.debug('verify method finished; amount of errors found: %s', err_count)
        else:
        ...
        if os.path.isdir(src):
            for dirname, subdirs, files in os.walk(src):
                ...
                for filename in files:
                    ...
                    logging.debug('        zipping %s as %s', os.path.join(dirname, filename),arcname)
        ...
        for path_s in self.source_paths:
            ...
            for path_d in self.destination_paths:
                ...
                if not os.path.exists(dst):
                    ...
                    err_count+=1                              
        logging.debug('verify method for .zip backup finished; amount of errors found: %s', err_count)                
        

    def zip_all(self,verify=False, timestamp=True):
        """compresses all the source files/folders into all the destination folders"""
        logging.debug('request received: to compress all source files and folders into all the destination folders')
        try:
            # checking paths
            if not self.paths_ok:
                raise Exception('files can\'t be compressed because paths have not been checked')
    ...
    def copy_all(self,verify=False,timestamp=False):
        ...
        logging.debug('request received: to copy all source files and folders into all the destination folders')
        try:
            # checking paths
            if not self.paths_ok:
                raise Exception('files can\'t be copied because paths have not been checked')
            ...
            for path_d in self.destination_paths:
                ...
                for path_s in self.source_paths:
                    ...
                    if os.path.isdir(path_s):
                        ...
                        logging.error('neither a folder or file - not copied: %s', path_s)
            # verifying
            if verify==True:
